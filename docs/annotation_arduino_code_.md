...
```flow
st=>start: arduino上电加载
e=>end: 关机
op1=>operation: 初始化串口
op1_1=>operation: 与processing建立连接
op2=>operation: 初始化眼球阵列
op3=>operation: 初始化舵机驱动板
op4=>operation: 初始化眼球阵列的板载ID与板内序号信息
op5=>operation: 初始化眼球位置
op6=>operation: 接收目标位置
op7=>operation: 发送准备接受位置标志位"B"
op8=>operation: 发送准备接受位置标志位"B"

sub1=>subroutine: 舵机转动到目标位置
cond=>condition: 接收到"$"字符

st->op1->op1_1->op2->op3->op4->op5->op8->cond
cond(no)->op8
cond(yes)->op6
op6->sub1()->op7(left)->cond
  ...
  if (Serial.available()) {
    ...
    if(a != 'A' && a == '$'){
       ...
       while(Serial.available()){
        ...
        if (inChar == '$'){
          pos_y = inString.toInt() - 136.5; //将[0,273]映射到[-273/2,273/2]
